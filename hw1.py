...
def apply_bias_trick(X):
    ...
    ###########################################################################
    numOfRows = X.shape[0]
    vectorBias = np.ones(numOfRows)
    biasX = np.column_stack((vectorBias, X))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return biasX
...
def compute_loss(X, y, theta):
    ...
    ###########################################################################
    dotResult = np.dot(X, theta)
    diffResult = dotResult - y
    J = np.sum(diffResult ** 2) / (2 * X.shape[0])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return J
...
def gradient_descent(X, y, theta, eta, num_iters):
    ...
    for i in range(num_iters):
        try:
            dotResult = np.dot(X, theta)
            differenceResult = dotResult - y
            outerDotResult = np.dot(X.T,differenceResult)
            grad = outerDotResult / X.shape[0]
            theta -= eta * grad
...
def compute_pinv(X, y):
    ...
    firstDotInverse = np.linalg.inv(firstDot)
    PinverseX = np.dot(firstDotInverse, X.T)
    pinv_theta = np.dot(PinverseX, y)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return pinv_theta
...
def gradient_descent_stop_condition(X, y, theta, eta, max_iter, epsilon=1e-8):
    ...
    for i in range(max_iter):
        ...
        differenceResult = dotResult - y
        outerDotResult = np.dot(X.T, differenceResult)
        theta -= (eta / X.shape[0]) * outerDotResult
        J_history.append(compute_loss(X, y, theta))
...
def find_best_learning_rate(X_train, y_train, X_val, y_val, iterations):
    ...
    ###########################################################################
    for eta in etas:
        initTheta = np.zeros(X_train.shape[1])
        theta, _ = gradient_descent(X_train, y_train, initTheta, eta, iterations)
        valLoss = compute_loss(X_val, y_val, theta)
        eta_dict[eta] = valLoss
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return eta_dict
    ...
    while len(selected_features) < 5 and len(selected_features) < num_features:
        ...
        for f in range(num_features):
            if f in selected_features:
                continue
            check_features = selected_features + [f]
            X_train_subset = X_train[:, check_features]
            X_val_subset = X_val[:, check_features]
            ...
            theta, _ = gradient_descent_stop_condition(X_train_bias, y_train, theta_init, best_eta, iterations)
            val_loss = compute_loss(X_val_bias, y_val, theta)
...
def create_square_features(df):
    ...
    for i in range(len(cols)):
        for j in range(i + 1, len(cols)):
            ...
            new_features[f"{col_i}*{col_j}"] = df[col_i] * df[col_j]
    poly_features_df = pd.DataFrame(new_features, index=df.index)
    df_poly = pd.concat([df_poly, poly_features_df], axis=1)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return df_poly