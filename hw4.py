...
class LogisticRegressionGD():
    ...
    def predict(self, X, threshold=0.5):
        ...
        y_pred = np.where(y_pred_bin == 1, self.class_names[1], self.class_names[0])
        ###########################################################################
        #                             END OF YOUR CODE                            #
        ###########################################################################
        return y_pred
    ...
    def BCE_loss(self, X, y):
        ...
        loss = -np.mean(y_01 * np.log(y_pred) + (1 - y_01) * np.log(1 - y_pred))
        ###########################################################################
        #                             END OF YOUR CODE                            #
        ###########################################################################
        return loss
    ...
    def fit(self, X, y):
        ...
        for i in range(self.max_iter):
            ...
            if i > 0 and abs(loss[-2] - loss[-1]) < self.eps:
                break

            gradient = np.dot(X.T, (probs - y_01)) / X.shape[0]
...
def select_learning_rate(X_train, y_train, learning_rates, max_iter):
    ...
    for eta in learning_rates:
        model = LogisticRegressionGD(learning_rate=eta, max_iter=max_iter)
...
def cv_accuracy_and_bce_error(X, y, n_folds):
    ...
    for i in range(n_folds):
        ...
        X_train = np.vstack([X_splits[j] for j in range(n_folds) if j != i])
        y_train = np.hstack([y_splits[j] for j in range(n_folds) if j != i])

        model = LogisticRegressionGD(learning_rate=0.0001, max_iter=10000)
        model.fit(X_train, y_train)

        y_pred = model.predict(X_val)
        acc = np.mean(y_pred == y_val)
        loss = model.BCE_loss(X_val, y_val)
...
def calc_and_print_metrics(y_true, y_pred, positive_class):
    ...
    # TODO: Implement the function in section below.                          #
    ###########################################################################
    y_true_to_bin = (y_true == positive_class).astype(int)
    y_pred_to_bin = (y_pred == positive_class).astype(int)

    tp = np.sum((y_true_to_bin == 1) & (y_pred_to_bin == 1))
    fp = np.sum((y_true_to_bin == 0) & (y_pred_to_bin == 1))
    tn = np.sum((y_true_to_bin == 0) & (y_pred_to_bin == 0))
    fn = np.sum((y_true_to_bin == 1) & (y_pred_to_bin == 0))
...
def fpr_tpr_per_threshold(y_true, positive_class_probs, positive_class="9"):
    ...
    for threshold in prob_thresholds:
        ...
        tpr_per_thresh = tp / (tp + fn) if (tp + fn) > 0 else 0
        fpr_per_thresh = fp / (fp + tn) if (fp + tn) > 0 else 0

        tpr.append(tpr_per_thresh)
        fpr.append(fpr_per_thresh)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return fpr, tpr